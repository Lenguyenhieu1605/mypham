...
namespace WebMyPham.Application.Catalog.Products
{
    ...
    public class ProductService : IProductService   //kế thừa từ Imanage
    {
        ...
        public async Task<ProductViewModel> GetById(int productId)
        {
            ...
            var image = await _context.ProductImages.Where(x => x.ProductId == productId && x.IsDefaut == true).FirstOrDefaultAsync();
            var productViewModel = new ProductViewModel()
            {
                ...
                Price = product.Price,
                Stock = product.Stock,
                Name = productdetail != null ? productdetail.Name : null,
                ViewCount = product.ViewCount,
                Description = productdetail != null ? productdetail.Description : null,
                Details = productdetail != null ? productdetail.Details : null,
        ...
        public async Task<List<ProductViewModel>> GetLatestProducts(int take)
        {
            ...
            var query = from p in _context.Products
                        ...
                        join c in _context.Categories on pic.ProductId equals c.Id into picc
                        from c in picc.DefaultIfEmpty()
                        where (pi == null || pi.IsDefaut == true)
                        select new { p, pd, pic, pi };
